...
void WebUtils::setHeaders(const vector<string> &headers) {
...
// Set post/put body
void WebUtils::setBody(const string &body) {
  curl_easy_setopt(curl, CURLOPT_POSTFIELDS, body.c_str());
  curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, body.size());
}

// callback function för att skriva till response. Spännande grunka
size_t WebUtils::writeCallback(void *contents, size_t size, size_t nmemb,
...
WebUtils::WebUtils() { // init libcurl
...
string WebUtils::post(const string &url, const vector<string> &headers,
                      const string &body) {
  if (!curl)
